...
const TodosLosFletes = ({ clienteId = '', fechaInicio = '', fechaFin = '' }) => {
...
// Estados para filtros - Inicializados con las props si existen
const [filters, setFilters] = useState({
  cliente: clienteId || "", // Inicializar con clienteId si existe
  codigo_flete: "",
  codigo_servicio: "",
  estado_flete: "",
  fecha_servicio_desde: fechaInicio || dateRange.fecha_servicio_desde, // Priorizar props
  fecha_servicio_hasta: fechaFin || dateRange.fecha_servicio_hasta, // Priorizar props
});
...
useEffect(() => {
  // Si hay props, NO sobreescribir con los valores de URL
...
}, [searchParams, getDateRangeFromMonthYear, hasInitialFilters]);
...
useEffect(() => {
  if (hasInitialFilters) {
    ...
    fetchFletes(1, pagination.itemsPerPage, {
      cliente: clienteId,
      fecha_servicio_desde: fechaInicio,
      fecha_servicio_hasta: fechaFin
    });
  } else {
...
const handleApplyFilters = useCallback(() => {
  ...
  // Si hay props, asegurar que los filtros de props siempre estén presentes
  if (hasInitialFilters) {
    ...
    setFilters(filtersWithProps);
    fetchFletes(1, pagination.itemsPerPage, filtersWithProps);
  } else {
    setFilters(localFilters);
    fetchFletes(1, pagination.itemsPerPage, localFilters);
  }
}, [localFilters, pagination.itemsPerPage, fetchFletes, hasInitialFilters, clienteId, fechaInicio, fechaFin]);
...
const clearFilters = useCallback(() => {
  // Si hay props, mantener los filtros de props
  if (hasInitialFilters) {
    const newFilters = {
      cliente: clienteId,
      codigo_flete: "",
    ...
    fetchFletes(1, pagination.itemsPerPage, newFilters);
    return;
  }

  // Si no hay props, usar el comportamiento original
  const mes = searchParams.get("mes") || "";
  const anio = searchParams.get("anio") || "";
...
}, [fetchFletes, pagination.itemsPerPage, searchParams, getDateRangeFromMonthYear, hasInitialFilters, clienteId, fechaInicio, fechaFin]);
    ...
    <div className="">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-6">
        <div>
          ...
          {(mesFromURL || anioFromURL || hasInitialFilters) && (
  <div className="mt-2">
    {hasInitialFilters ? (
      <>
