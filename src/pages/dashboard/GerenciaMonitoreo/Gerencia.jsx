...
const Gerencia = () => {
  ...
  const toggleFreightDetails = useCallback(async (index, freight) => {
    ...
    if (expandedFreightIndex === index) {
      ...
      // Si no está expandido, expandir y cargar gastos si no se han cargado
      setExpandedFreightIndex(index);
      
      // Cargar gastos si no se han cargado antes
            ...
            <tbody>
              {gastos.map((gasto, index) => (
                <tr key={gasto.id || index} className="border-b border-gray-200 hover:bg-gray-50">
                  ...
                  <td className="py-1 px-2 border border-gray-200 max-w-[150px] truncate">
                    {gasto.descripcion}
                  </td>
                  ...
                  <td className="py-1 px-2 border border-gray-200">
                    <span className={`inline-block px-1.5 py-0.5 rounded text-xs border ${
                      gasto.estado_facturacion === 'Pagado' 
                        ? 'bg-green-50 text-green-700 border-green-200'
                        : gasto.estado_facturacion === 'Pendiente'
                        ? 'bg-yellow-50 text-yellow-700 border-yellow-200'
                        : 'bg-gray-50 text-gray-700 border-gray-200'
                    }`}>
                      {gasto.estado_facturacion || 'N/A'}
                    </span>
                  </td>
                  <td className="py-1 px-2 border border-gray-200">
                    <span className={`inline-block px-1.5 py-0.5 rounded text-xs border ${
                      gasto.estado_aprobacion === 'aprobado'
                        ? 'bg-green-50 text-green-700 border-green-200'
                        : gasto.estado_aprobacion === 'rechazado'
                        ? 'bg-red-50 text-red-700 border-red-200'
                        : 'bg-yellow-50 text-yellow-700 border-yellow-200'
                    }`}>
  ...
  const renderFreightDetails = (freight, index) => {
    ...
    const servicio = freight.servicio || {};
    const fleteId = freight.id || freight.codigo_flete;
    const hasExpenses = freightExpenses[fleteId]?.cantidad_gastos > 0;
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div className="min-w-0">
              <div className="flex items-center space-x-2">
                <span className="font-medium text-sm text-gray-900 truncate">{freight.codigo_flete}</span>
                <span className="text-xs text-gray-500 truncate">{servicio.destino}</span>
              </div>
            </div>
          ...
          <div className="flex items-center space-x-2">
            {/* Mostrar indicador de gastos si existen */}
            {hasExpenses && (
              <span className="text-xs px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded flex items-center flex-shrink-0">
                <CreditCard className="h-2.5 w-2.5 mr-1 flex-shrink-0" />
                {freightExpenses[fleteId].cantidad_gastos} gastos
              </span>
            )}
            <span className="text-xs px-2 py-1 bg-blue-50 text-blue-700 rounded flex-shrink-0">
              {servicio.tipo_servicio || 'Local'}
            </span>
            <ChevronDown className={`h-4 w-4 text-gray-500 transition-transform flex-shrink-0 ${isExpanded ? 'rotate-180' : ''}`} />
          </div>
        </div>
