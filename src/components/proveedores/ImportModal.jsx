...
const ImportModal = ({
  ...
  const handleDownloadTemplate = useCallback(async () => {
    try {
      const blob = await proveedoresAPI.downloadPlantillaExcel();
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      const result = await proveedoresAPI.importProveedoresExcel(selectedFile);
  ...
  const handleReset = () => {
    ...
    setError(null);
    setStep(1);
    if (document.getElementById('file-input-proveedores')) {
      document.getElementById('file-input-proveedores').value = '';
    }
  };
  ...
  const calculateSuccessRate = (result) => {
    ...
    return `${((totalProcessed / totalRows) * 100).toFixed(1)}%`;
  };
      ...
      <div className="space-y-6">
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <ul className="text-sm text-blue-700 space-y-1">
                    ...
                    <li>• Las columnas obligatorias son: Tipo Documento, Número Documento, Razón Social, Rubro</li>
                    <li>• Los servicios deben separarse con punto y coma (;)</li>
                    <li>• Los contactos y cuentas deben seguir el formato de la plantilla</li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-300 rounded-lg">
              ...
              <div className="w-full">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <Button
                    type="button"
                    onClick={() => document.getElementById('file-input-proveedores').click()}
                    variant="secondary"
                    icon={Upload}
                    disabled={isLoading}
                  >
        ...
        {step === 2 && importResult && (
          <div className="space-y-4">
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className={`text-sm ${
                    ...
                    Tasa de éxito: {calculateSuccessRate(importResult.result)}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
                  <h4 className="font-medium text-gray-700">
                    Errores encontrados ({importResult.result.errors.length}):
                  </h4>
                </div>
            ...
            {/* Información adicional de errores si existe */}
            {importResult.result?.error_details && importResult.result.error_details.length > 0 && (
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
                  <h4 className="font-medium text-gray-700">Detalles de errores:</h4>
                </div>
                <div className="max-h-60 overflow-y-auto">
                  <ul className="divide-y divide-gray-200">
                    {importResult.result.error_details.map((errorDetail, index) => (
                      <li key={index} className="px-4 py-3 hover:bg-gray-50">
                        <div className="flex items-start">
                          <AlertCircle className="h-4 w-4 text-orange-500 mr-2 flex-shrink-0 mt-0.5" />
                          <div className="text-sm">
                            <p className="font-medium text-gray-700">Fila {errorDetail.row}:</p>
                            <p className="text-gray-600 mt-1">{errorDetail.error}</p>
                            {errorDetail.field && (
                              <p className="text-xs text-gray-500 mt-1">Campo: {errorDetail.field}</p>
                            )}
                          </div>
                        </div>
                      </li>
