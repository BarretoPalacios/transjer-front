...
import { flotaAPI } from '../../api/endpoints/flotas'; // Asegúrate de que la ruta sea correcta
...
const ImportModal = ({
  ...
  const handleDownloadTemplate = useCallback(async () => {
    try {
      const blob = await flotaAPI.downloadPlantillaExcel();
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      const result = await flotaAPI.importFlotasExcel(selectedFile);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title="Importar Flotas desde Excel"
      size="large"
      {...props}
    >
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <ul className="text-sm text-blue-700 space-y-1">
                    ...
                    <li>• Columnas importantes: Código Flota, Placa, Marca, Modelo</li>
                    <li>• Verifica los tipos de datos (año numérico, activo booleano)</li>
                  </ul>
                </div>
              </div>
            </div>
