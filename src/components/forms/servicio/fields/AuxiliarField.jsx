// components/forms/servicio/fields/AuxiliarField.jsx - ACTUALIZADO
...
const AuxiliarField = ({ 
  ...
  const fetchAuxiliares = useCallback(async (search = '') => {
    try {
      ...
      console.log('Auxiliares obtenidos:', items);
      ...
      console.error('Error buscando auxiliares:', error);
      ...
      >
        <div className="flex flex-col">
          ...
          <div className="text-xs text-gray-500 flex justify-between mt-0.5">
            ...
            {option.telefono && <span>Tel: {option.telefono}</span>}
          </div>
          {option.turno && (
  ...
  const handleAuxiliarCreated = useCallback((nuevoAuxiliar) => {
    const newValue = [...(value || []), nuevoAuxiliar];
    onChange(newValue);
    setShowModal(false);
  }, [onChange, value]);
    ...
    <>
      ...
      {showModal && (
        <QuickPersonalModal
          ...
          onSuccess={handleAuxiliarCreated}
