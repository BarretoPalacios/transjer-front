// components/forms/servicio/fields/ModalidadField.jsx
import React, { useState, useCallback } from 'react';
import BaseField from './BaseField';
import AdvancedSearchDropdown from '../AdvancedSearchDropdown';

const ModalidadField = ({ 
  value, 
  onChange, 
  error, 
  disabled = false,
  ...
  const modalidades = [
    ...
    { id: 'DESCARGA', nombre: 'DESCARGA' },
    { id: 'CARGA/DESCARGA', nombre: 'CARGA/DESCARGA' },
  ...
  const fetchOptions = useCallback(async (search = '') => {
    const filtered = search
      ? modalidades.filter(modalidad => 
          modalidad.nombre.toLowerCase().includes(search.toLowerCase())
        )
      : modalidades;
    ...
    return (
      <div
        ...
        className={`px-2 py-1.5 cursor-pointer text-xs transition-colors ${
          isOther ? 'hover:bg-yellow-50' : 'hover:bg-purple-50'
        } text-gray-700`}
      >
      ...
      <BaseField label={label} error={error} required={required}>
        <div className="flex space-x-1">
          <input
            ...
            placeholder="Ingrese modalidad personalizada..."
            disabled={disabled}
