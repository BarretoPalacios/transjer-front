...
export const fletesAPI = {
  ...
  getAllFletes: async (filters = {}) => {
    ...
    if (filters.codigo_servicio)
      params.append("codigo_servicio", filters.codigo_servicio);
    ...
    if (filters.fecha_creacion)
      params.append("fecha_creacion", filters.fecha_creacion);
    if (filters.fecha_creacion_desde)
      params.append("fecha_creacion_desde", filters.fecha_creacion_desde);
    if (filters.fecha_creacion_hasta)
      params.append("fecha_creacion_hasta", filters.fecha_creacion_hasta);
  ...
  getFletesByFechaServicio: async (
    fechaDesde,
    fechaHasta,
    additionalFilters = {},
  ) => {
  ...
  getPendientesFacturacion: async (filters = {}) => {
    ...
    if (filters.fecha_servicio_hasta)
      params.append("fecha_servicio_hasta", filters.fecha_servicio_hasta);

    const response = await axiosInstance.get(
      `/fletes/stats/pendientes-facturacion?${params.toString()}`,
    );
    return response.data;
  },
  ...
  exportFletesExcel: async (filters = {}) => {
    try {
      ...
      const hasServiceFilters =
        ...
        filters.fecha_servicio_desde ||
        filters.fecha_servicio_hasta;
      ...
      const excelData = (fletesData.items || []).map((flete) => {
        const baseData = {
          ...
          "Fecha Creación": flete.fecha_creacion
            ? new Date(flete.fecha_creacion).toLocaleDateString()
            : "-",
        };
        ...
        if (flete.servicio) {
          return {
            ...
            "Fecha Servicio": flete.servicio.fecha_servicio
              ? new Date(flete.servicio.fecha_servicio).toLocaleDateString()
              : "-",
  ...
  exportAllFletesExcel: async (filters = {}) => {
    ...
    if (filters.pertenece_a_factura !== undefined)
      params.append("pertenece_a_factura", filters.pertenece_a_factura);
    if (filters.cliente_nombre)
      params.append("cliente_nombre", filters.cliente_nombre);
  ...
  getGastosByFlete: async (codigoFlete) => {
    const response = await axiosInstance.get(
      `/gastos-adicionales/flete/${codigoFlete}`,
    );
    return response.data;
  },
  ...
  getGastosByCodeFlete: async (codigoFlete) => {
    const response = await axiosInstance.get(
      `/gastos-adicionales/flete-code/${codigoFlete}`,
    );
    return response.data;
  },
...
export const buildAdvancedFleteFilters = (filters) => {
  ...
  if (filters.pertenece_a_factura !== undefined)
    cleanFilters.pertenece_a_factura = filters.pertenece_a_factura;
  if (filters.codigo_factura)
    cleanFilters.codigo_factura = filters.codigo_factura;
  if (filters.monto_min !== undefined && filters.monto_min !== null)
    cleanFilters.monto_min = filters.monto_min;
  if (filters.monto_max !== undefined && filters.monto_max !== null)
    cleanFilters.monto_max = filters.monto_max;
  ...
  if (filters.estado_servicio)
    cleanFilters.estado_servicio = filters.estado_servicio;

  // Filtros de Fecha
  if (filters.fecha_servicio_desde)
    cleanFilters.fecha_servicio_desde = formatDateToISO(
      filters.fecha_servicio_desde,
    );
  if (filters.fecha_servicio_hasta)
    cleanFilters.fecha_servicio_hasta = formatDateToISO(
      filters.fecha_servicio_hasta,
    );
  if (filters.fecha_creacion_desde)
    cleanFilters.fecha_creacion_desde = formatDateToISO(
      filters.fecha_creacion_desde,
    );
  if (filters.fecha_creacion_hasta)
    cleanFilters.fecha_creacion_hasta = formatDateToISO(
